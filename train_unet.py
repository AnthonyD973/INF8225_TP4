...
dataset_train = RandomResizeDataset(torchvision.datasets.FashionMNIST(args.data_dir + "/train", train=True, download=True), ratio_mu=1.0, ratio_sigma=0.25)
loader_train = torch.utils.data.DataLoader(dataset_train, batch_size=60)

dataset_test = RandomResizeDataset(torchvision.datasets.FashionMNIST(args.data_dir + "/test", train=False, download=True), ratio_mu=1.0, ratio_sigma=0.25)
loader_test = torch.utils.data.DataLoader(dataset_test, batch_size=60)

net = RecurrentAE(3).to(torch.double)
